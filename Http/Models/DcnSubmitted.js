...
update_details = function (req, res, index) {
    ...
    new sql.connect(config).then(pool => {
        return pool.request()
                    ...
                    .input('TimeOut4', sql.VarChar(20), JSON.parse(req.body.TimeOut4)[ii] ? JSON.parse(req.body.TimeOut4)[ii].toString() : '')
                    .input('HoursPerDay', sql.Float, req.body.HoursPerDay ? parseFloat(JSON.parse(req.body.HoursPerDay)[ii]) : 0)
                    .input('MobilityWalkingMovingFlag', sql.Bit, (JSON.parse(req.body.MobilityWalkingMovingFlag)[ii].toString() == 'true'))
                    .input('BathingShoweringFlag', sql.Bit, (JSON.parse(req.body.BathingShoweringFlag)[ii].toString() == 'true'))
                    .input('DressingFlag', sql.Bit, (JSON.parse(req.body.DressingFlag)[ii].toString() == 'true'))
                    .input('ToiletingFlag', sql.Bit, (JSON.parse(req.body.ToiletingFlag)[ii].toString() == 'true'))
                    .input('EatingFlag', sql.Bit, (JSON.parse(req.body.EatingFlag)[ii].toString() == 'true'))
                    .input('ContinenceBladderBowelFlag', sql.Bit, (JSON.parse(req.body.ContinenceBladderBowelFlag)[ii].toString() == 'true'))
                    .input('MealPrepIncludingFlag', sql.Bit, (JSON.parse(req.body.MealPrepIncludingFlag)[ii].toString() == 'true'))
                    .input('LaundryFlag', sql.Bit, (JSON.parse(req.body.LaundryFlag)[ii].toString() == 'true'))
                    .input('LightHousekeepingIncludingFlag', sql.Bit, (JSON.parse(req.body.LightHousekeepingIncludingFlag)[ii].toString() == 'true'))
                    .input('PersonalCareHours', sql.Int, req.body.PersonalCareHours ? parseInt(req.body.PersonalCareHours) : 0)
                    .input('HomemakingHours', sql.Int, req.body.HomemakingHours ? parseInt(req.body.HomemakingHours) : 0)
                    .input('CompanionHours', sql.Int, req.body.CompanionHours ? parseInt(req.body.CompanionHours) : 0)
                    .input('RespiteHours', sql.Int, req.body.RespiteHours ? parseInt(req.body.RespiteHours) : 0)
